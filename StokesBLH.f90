      real*8 function StokesBLH(BLH,gra,sfh,nlat,nlon,hd,dr,GRS)
      !the generalized Stokes/Hotine rigorous numerical integral 广义Stokes积分积分
      !dr-积分半径 the integral radius (m)
!-------------------------------------------------------------
      implicit none
	integer::i,j,nlat,nlon,i0,j0,ni,nj
	real*8::dr,gra(nlat,nlon),sfh(nlat,nlon)
	real*8::hd(6),pi,RAD,ds,mdr,tt,rr,r0,r1,r2
	real*8::GRS(6),BLH(3),XYZ(3),rln(3),BLH0(3),XYZ0(3),BLH1(3),XYZ1(3),rln1(3)
	real*8 CGrdPntD2,StokesSgn,L0,L1,SK,rst,gr,NFD(5),Stkhtn0,gra0,tmp
!-----------------------------------------------------------------
      pi=datan(1.d0)*4.d0;RAD=pi/180.d0;BLH0=BLH;rst=0.d0
      BLH0(3)=CGrdPntD2(BLH(2),BLH(1),sfh,nlat,nlon,hd)
	  ...
	  do j=j0-nj,j0+nj
	    ...
	    BLH1(2)=hd(1)+(real(j)-0.5d0)*hd(5)
          ...
          if(L0<mdr)then!计算奇异积分
             rst=rst+StokesSgn(BLH,gra,sfh,nlat,nlon,hd,i,j,4,GRS)
             goto 9101 
          endif
          call BLH_RLAT(GRS,BLH1,rln1);r1=rln1(1)
          tt=1.d0-2.d0*(L0/r1/2.d0)**2
          ds=hd(5)*hd(6)*RAD**2*dcos(rln1(2)*RAD)
          SK=2.d0/L1+1.d0/rr-3.d0*L1/rr**2-5.d0*r1*tt/rr**2
          SK=SK-3.d0*r1/rr**2*tt*dlog(dabs(rr+L1-r1*tt)/2.d0/rr)
...
!--------------------------------------------------------------------------------
      real*8 function StokesSgn(BLH,gra,sfh,nlat,nlon,hd,i0,j0,m,GRS)
      !细化核函数，计算BLH点的Stokes奇异积分
      !m-核函数细化为m*m
      !i0,j0-奇异点格网位置
!-------------------------------------------------------------
      implicit none
...
!-----------------------------------------------------------------
      ...
      rst=0.d0;mdr=r0*rv*RAD*dcos(rln0(2)*RAD)/4.d0 !奇异点判断
	do i=1,m
	  ...
	  do j=1,m
	    BLH1(2)=lon+(real(j)-0.5d0)*rv
          ...
          SK=2.d0/L1+1.d0/rr-3.d0*L1/rr**2-5.d0*r1*tt/rr**2-3.d0*r1/rr**2*tt*dlog(dabs(rr+L1-r1*tt)/2.d0/rr)
